...
@onready var ui: CanvasLayer = preload("res://Player/Scene/GameUI.tscn").instantiate()
...
var flashlight_instance: Node3D = null
...
func _input(event):
	if event.is_action_pressed("flashlight_toggle") and has_flashlight:
		flashlight_instance.toggle()  # Вызываем метод toggle у фонарика
...
func _physics_process(delta):
	if ray.is_colliding():
		...
		if collider.is_in_group("Interactable") and collider.has_method("interact"):
			...
			ui.show_interaction_prompt(collider.interaction_prompt)
		else:
...
func equip_flashlight(flashlight_scene: PackedScene):
	if !has_flashlight:
		flashlight_instance = flashlight_scene.instantiate()
		$Head/Camera3D.add_child(flashlight_instance)
		has_flashlight = true
		flashlight_instance.visible = false  # Начальное состояние выключен
